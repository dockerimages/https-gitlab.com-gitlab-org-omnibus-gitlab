...
class PackageSizeCheck
  ...
  class << self
    def fetch_sizefile
      ...
      pipeline_jobs = gitlab_client.pipeline_jobs(project_id, pipeline_id).auto_paginate

      trigger_package_job = pipeline_jobs.find { |j| j.name == 'Ubuntu-20.04-branch' }

      folder = 'ubuntu-focal'
      folder = "#{folder}_fips" if Build::Check.use_system_ssl?
      package_filename_url_safe = Build::Info.release_version.gsub("+", "%2B")
      ...
      sizefile_url = URI("#{api_url}/projects/#{project_id}/jobs/#{trigger_package_job.id}/artifacts/pkg/#{folder}/#{Build::Info.package}_#{package_filename_url_safe}_amd64.deb.size")
