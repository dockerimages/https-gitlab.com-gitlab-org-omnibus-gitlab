...
module Build
  class Info
    class << self
      ...
      def image_reference
        if Gitlab::Util.get_env('CI_PIPELINE_TRIGGERED') && Gitlab::Util.get_env('CI_PIPELINE_SOURCE') == 'trigger'
          "#{Build::GitlabImage.gitlab_registry_image_address}:#{Gitlab::Util.get_env('IMAGE_TAG')}"
        elsif Build::Check.is_nightly?
        ...
        elsif Gitlab::Util.get_env('CI_COMMIT_TAG')
          "#{Build::GitlabImage.gitlab_registry_image_address}:#{Info.docker_tag}"
        else
