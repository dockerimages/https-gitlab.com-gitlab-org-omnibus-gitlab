...
namespace :qa do
  ...
  task test: ["qa:build", "qa:push:triggered"] do # Requires the QA image to be built and pushed first
    ...
    qa_image_address = Build::QAImage.gitlab_registry_image_address(tag: ENV['IMAGE_TAG'])
    Build::QATrigger.new(image: image_address, qa_image: qa_image_address).invoke!.wait!
  end
end
