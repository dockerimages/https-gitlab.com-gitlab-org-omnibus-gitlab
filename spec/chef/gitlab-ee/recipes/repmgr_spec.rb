...
describe 'repmgr' do
  let(:chef_run) { ChefSpec::SoloRunner.new(step_into: %w(gitlab_service runit_service postgresql_config)).converge('gitlab-ee::default') }
...
witness_repl_nodes_sync_interval_secs=15
  ...
  context 'repmgrd_disable' do
    let(:chef_run) { ChefSpec::SoloRunner.new(step_into: %w(gitlab_service runit_service)).converge('repmgr::repmgrd_disable') }
    it_behaves_like 'disabled runit service', 'repmgrd'
  end

  context 'repmgrd' do
    let(:chef_run) { ChefSpec::SoloRunner.new(step_into: %w(gitlab_service runit_service)).converge('repmgr::repmgrd') }
