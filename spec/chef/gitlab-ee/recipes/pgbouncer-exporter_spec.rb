...
describe 'gitlab-ee::pgbouncer-exporter' do
  ...
  describe 'when enabled' do
    ...
    it 'creates the appropriate directories' do
      ...
      expect(chef_run).to create_directory('/var/opt/gitlab/pgbouncer-exporter')
    end

    it 'installs pgbouncer-exporter.yaml with default values' do
      expect(chef_run).to render_file(config_yaml).with_content { |content|
        expect(content).to match(/^exporter_port: 9188$/)
      }
    end

    context 'config YAML changes' do
      let(:template) { chef_run.template(config_yaml) }
      ...
      it 'reloads pgbouncer and starts pgbouncer if it is not running' do
        ...
        allow_any_instance_of(OmnibusHelper).to receive(:should_notify?).with('pgbouncer-exporter').and_return(true)
        expect(template).to notify('service[pgbouncer-exporter]').to(:restart).immediately
      end
    end
  end
end
