...
RSpec.describe 'praefect' do
  ...
  context 'when manage database is enabled and geo role is enabled' do
    before do
      stub_gitlab_rb(
    ...
    it 'should not create database and users' do
      expect { ChefSpec::SoloRunner.new.converge('gitlab-ee::default') }.to raise_error(/cofigure Praefect database/)
    end
  end
end
