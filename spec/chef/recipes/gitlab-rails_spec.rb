...
describe 'gitlab::gitlab-rails' do
  ...
  describe "with symlinked templates" do
    ...
    describe 'gitlab_pages_shared_secret' do
      let(:templatesymlink) { chef_run.templatesymlink('Create a gitlab_pages_shared_secret and create a symlink to Rails root') }
      ...
      context 'by default' do
        ...
        it 'creates the template' do
          expect(chef_run).to create_templatesymlink("Create a gitlab_pages_shared_secret and create a symlink to Rails root").with(
            owner: 'root',
            group: 'root',
            mode: '0644'
          )
      ...
      context 'with specific gitlab_pages_shared_secret' do
        let(:shared_secret) { SecureRandom.base64(32) }
        cached(:chef_run) do
          RSpec::Mocks.with_temporary_scope do
            stub_gitlab_rb(
              gitlab_pages: { shared_secret: shared_secret },
              external_url: 'http://gitlab.example.com',
              pages_external_url: 'http://pages.example.com'
            )
          end
        ...
        it 'renders the correct node attribute' do
          expect(chef_run).to create_templatesymlink("Create a gitlab_pages_shared_secret and create a symlink to Rails root").with_variables(
