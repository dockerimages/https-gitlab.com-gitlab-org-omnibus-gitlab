  ...
  describe 'GitLab Shell settings' do
    context 'with default values' do
      it 'renders gitlab.yml with default values' do
        expect(generated_yml_content[:production][:gitlab_shell]).to eq(
    ...
    context 'with user specified values' do
      ...
      describe 'authorized_keys_file' do
        context 'specified directly' do
          ...
          it 'renders gitlab.yml with user specified values' do
            expect(generated_yml_content[:production][:gitlab_shell][:authorized_keys_file]).to eq('/tmp/authorized_keys')
          end
        end

        context 'user home directory specified' do
          before do
            stub_gitlab_rb(
              user: {
                home: '/tmp/user'
          ...
          it 'defaults to auth file within specified home directory' do
            expect(generated_yml_content[:production][:gitlab_shell][:authorized_keys_file]).to eq('/tmp/user/.ssh/authorized_keys')
          end
        end
      end
    end
