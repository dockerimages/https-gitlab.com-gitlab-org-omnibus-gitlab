...
RSpec.describe 'gitlab::gitlab-rails' do
  ...
  describe 'omniauth settings' do
    context 'with default values' do
      it 'renders gitlab.yml with omniauth disabled' do
        expect(generated_yml_content[:production][:omniauth]).to be nil
      end
    end
        ...
        with_them do
          context "with #{params[:gitlab_rb_setting]} set to true" do
            ...
            it "renders gitlab.yml with #{params[:gitlab_yml_setting]} set to true" do
              expect(generated_yml_content[:production][:omniauth][gitlab_yml_setting.to_sym]).to be true
            end
          end
        end
      end
          ...
          [true, 'foo', ['bar', 'tar']].each do |value|
            context "with #{params[:gitlab_rb_setting]} set to #{value}" do
              ...
              it "renders gitlab.yml with #{params[:gitlab_yml_setting]} set to #{value}" do
                expect(generated_yml_content[:production][:omniauth][gitlab_yml_setting.to_sym]).to eq value
              end
            end
          end
        end
