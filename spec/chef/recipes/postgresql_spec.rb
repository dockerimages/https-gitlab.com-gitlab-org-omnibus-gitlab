...
RSpec.describe 'postgresql 9.6' do
  ...
  context 'version specific settings' do
    ...
    it 'creates the btree_gin extension when it is possible' do
      allow_any_instance_of(PgHelper).to receive(:extension_can_be_enabled?).with('btree_gin', 'gitlabhq_production').and_return(true)
      expect(chef_run).to enable_postgresql_extension('btree_gin')
    end

    it 'does not create the btree_gin extension if it is not possible' do
      allow_any_instance_of(PgHelper).to receive(:extension_can_be_enabled?).with('btree_gin', 'gitlabhq_production').and_return(false)
      expect(chef_run).not_to run_execute('enable btree_gin extension')
    end
