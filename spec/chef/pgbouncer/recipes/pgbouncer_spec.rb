...
RSpec.describe 'pgbouncer' do
  ...
  let(:pgbouncer_ini) { '/var/opt/gitlab/pgbouncer/pgbouncer.1.ini' }
  ...
  describe 'when enabled' do
    ...
    it 'installs pgbouncer.ini with default values' do
      ...
      expect(chef_run).to render_file(pgbouncer_ini).with_content { |content|
        ...
        expect(content).to match(%r{^unix_socket_dir = /var/opt/gitlab/pgbouncer/1$})
        expect(content).to match(%r{^pidfile = /var/opt/gitlab/pgbouncer/1/pgbouncer.pid$})
    ...
    context 'pgbouncer.ini template changes' do
      ...
      it 'stores the socket directory in a different location when set' do
        ...
        expect(chef_run).to render_file(pgbouncer_ini).with_content { |content|
          expect(content).to match(%r{^unix_socket_dir = /fake/dir/1$})
