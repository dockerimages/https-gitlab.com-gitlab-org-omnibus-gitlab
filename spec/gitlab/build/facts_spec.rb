...
RSpec.describe Build::Facts do
  ...
  describe '.generate_env_file' do
    before do
      allow(described_class).to receive(:common_vars).and_return(%w[TRIGGERED_USER=JohnDoe])
    ...
    it 'writes environment variables to file' do
      expect(File).to receive(:write).with('build_facts/env_vars', "TRIGGERED_USER=JohnDoe\nQA_RELEASE=foobar")
  ...
  describe '.common_vars' do
    before do
      stub_env_var('TRIGGERED_USER', 'JohnDoe')
      stub_env_var('TRIGGER_SOURCE', 'foobar')
    ...
    it 'returns correct variables' do
      result = %w[
        TRIGGERED_USER=JohnDoe
        TRIGGER_SOURCE=foobar
