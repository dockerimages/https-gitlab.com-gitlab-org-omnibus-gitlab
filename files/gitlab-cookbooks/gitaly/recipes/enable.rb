...
log_directory = node['gitaly']['configuration']['logging']['dir']
...
json_logging = node['gitaly']['configuration']['logging']['format'].eql?('json')
open_files_ulimit = node['gitaly']['open_files_ulimit']
runtime_dir = node['gitaly']['configuration']['runtime_dir']
cgroups_mountpoint = node['gitaly']['configuration']['cgroups']['mountpoint']
cgroups_hierarchy_root = node['gitaly']['configuration']['cgroups']['hierarchy_root']
...
template "Create Gitaly config.toml" do
  ...
  variables node['gitaly'].to_hash.merge(
    {
      configuration: node['gitaly']['configuration'].merge(
        {
...
consul_service node['gitaly']['consul_service_name'] do
  ...
  socket_address node['gitaly']['configuration']['prometheus_listen_addr']
