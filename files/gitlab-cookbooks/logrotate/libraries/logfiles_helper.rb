...
class LogfilesHelper < AccountHelper
  ...
  def service_settings(service)
    ...
    elsif parent = service_parent(service)
      node[parent][service]
    else
      node[service]
    end
  ...
  def logdir(service)
    ...
    log_directory_key = if service == 'mattermost'
                          'log_file_directory'
                        else
                          'log_directory'
                        end
    service_settings(service)[log_directory_key]
  end
  ...
  def logdir_owner(service)
    unless default_logdir_ownership.key?(service)
      Chef::Log.warn("#{service} does not have an owner user defined for its log directory. Setting to root.")
      return 'root'
    end

    default_logdir_ownership[service][:username] || 'root'
  end
