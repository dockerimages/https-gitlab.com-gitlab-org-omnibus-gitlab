...
dependent_services = []
dependent_services << "gitlab_service[mailroom]" if node['gitlab']['mailroom']['enable']

node['gitlab']['gitlab-rails']['dependent_services'].each do |name|
  dependent_services << "gitlab_service[#{name}]" if omnibus_helper.should_notify?(name)
end
...
gitlab_workhorse_services += ['gitlab_service[gitlab-workhorse]'] if omnibus_helper.should_notify?('gitlab-workhorse')
...
gitlab_pages_services += ['gitlab_service[gitlab-pages]'] if omnibus_helper.should_notify?('gitlab-pages')
