...
gitlab_sshd_generate_host_keys = node['gitlab']['gitlab-sshd']['generate_host_keys']
gitlab_sshd_bin_path = File.join(gitlab_shell_dir, 'bin', 'gitlab-sshd')
gitlab_sshd_working_dir = node['gitlab']['gitlab-sshd']['dir']
gitlab_sshd_log_dir = node['gitlab']['gitlab-sshd']['log_directory']
gitlab_sshd_env_dir = node['gitlab']['gitlab-sshd']['env_directory']
gitlab_sshd_host_key_dir = node['gitlab']['gitlab-sshd']['host_keys_dir']
...
sshd_config = GitlabSshdHelper.new(node)
...
templatesymlink "Create a config.yml and create a symlink to Rails root" do
  ...
  variables({
              ...
              gitlab_sshd: sshd_config.json_config
            })
...
env_dir gitlab_sshd_env_dir do
  ...
  variables node['gitlab']['gitlab-sshd']['env']
  notifies :restart, "runit_service[gitlab-sshd]" if omnibus_helper.should_notify?('gitlab-sshd')
end
