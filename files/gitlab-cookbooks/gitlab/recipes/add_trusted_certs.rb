...
ruby_block "Move existing certs and link to #{ssl_certs_dir}" do
  ...
  notifies :restart, "gitlab_service[unicorn]" if omnibus_helper.should_notify?("unicorn")
  notifies :restart, "gitlab_service[puma]" if omnibus_helper.should_notify?("puma")
  notifies :restart, "gitlab_service[gitlab-pages]" if omnibus_helper.should_notify?("gitlab-pages")
end
