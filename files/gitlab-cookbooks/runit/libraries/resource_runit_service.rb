      ...
      def after_created
        unless run_context.nil?
          ...
          find_resource(:service, new_resource.name) do # creates if it does not exist
            ...
            start_command "#{new_resource.sv_bin} #{new_resource.start_command_name} #{service_dir_name}"
            stop_command "#{new_resource.sv_bin} #{new_resource.stop_command_name} #{service_dir_name}"
            restart_command "#{new_resource.sv_bin} #{new_resource.restart_command_name} #{service_dir_name}"
            status_command "#{new_resource.sv_bin} #{new_resource.status_command_name} #{service_dir_name}"
            action :nothing
          end
        end
      end
