...
build do
  ...
  project_path_opt = gitlab_bundle_gemfile == 'Gemfile' ? './' : gitlab_bundle_gemfile.delete_suffix('Gemfile')
  bundle "exec license_finder report --project_path=#{project_path_opt} --decisions-file=config/dependency_decisions.yml --format=json --columns name version licenses texts notice --save=rails-license.json", env: env
